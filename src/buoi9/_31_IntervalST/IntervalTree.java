...
public class IntervalTree<Value> {
    ...
    public static void main(String[] args) {
        ...
        try {
            File file = new File("src/buoi9/interval.txt");
            ...
            while (sc.hasNext()) {
                if (sc.hasNextDouble()) {
                    ...
                    tree.put(new Interval1D(min, max), "Interval-" + id++);
                }
            }
            sc.close();
        ...
        testSearch(tree, 18.0, 22.0);
        testSearchAll(tree, 16.0, 18.0);
        testSearch(tree, 100.0, 200.0);
    }

    private static void testSearch(IntervalTree<String> tree, double min, double max) {
        Interval1D query = new Interval1D(min, max);
        System.out.print("search" + query + ": ");
        Interval1D result = tree.search(query);

        if (result != null) System.out.println(result);
        else System.out.println("null");
    }

    private static void testSearchAll(IntervalTree<String> tree, double min, double max) {
        Interval1D query = new Interval1D(min, max);
        System.out.print("searchAll" + query + ":");
        for (Interval1D interval : tree.searchAll(query)) {
